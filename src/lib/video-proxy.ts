...
export async function validateVideoAccess(
  ...
  try {
    const { data: lesson, error: lessonErr } = await adminSupabase
      .from('lessons')
      .select('id, title, content_type, is_active, is_free, module_id')
      .eq('id', lessonId)
      .maybeSingle()
    ...
    if (!lesson.is_active) {
      return { allowed: false, error: 'Lesson is not available', status: 403 }
    }
    if (lesson.content_type !== 'video') {
      return { allowed: false, error: 'Lesson is not a video', status: 400 }
    }
    ...
    if (!module) {
      return { allowed: false, error: 'Module not found', status: 404 }
    }
    ...
    if (!course) {
      return { allowed: false, error: 'Course not found', status: 404 }
    }
    if (!course.is_active) {
      return { allowed: false, error: 'Course is not available', status: 403 }
    }

    if (lesson.is_free) {
      return { allowed: true, courseId: course.id, courseTitle: course.title, lessonTitle: lesson.title }
    }
...
export async function getLessonVideoInfo(lessonId: string): Promise<{
  ...
  const { data, error } = await adminSupabase
    .from('lessons')
    .select('video_id, r2_source_key, stream_status, module_id')
    .eq('id', lessonId)
    .maybeSingle()
  ...
  return {
    ...
    r2SourceKey: data.r2_source_key,
