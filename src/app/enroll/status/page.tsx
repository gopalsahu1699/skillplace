...
export const metadata: Metadata = {
  title: 'Enrollment Status | Skillplace Academy',
  description: 'Check the status of your enrollment.',
  robots: { index: false, follow: false },
}

export default async function EnrollStatusPage(props: { searchParams: Promise<{ order_id?: string; status?: string }> }) {
    ...
    <div className="min-h-screen bg-surface flex items-center justify-center px-margin-mobile">
      <div className="text-center max-w-lg">
        ...
        <h1 className="font-display-lg text-display-lg-mobile md:text-display-lg text-primary mb-4">
          Enrollment {status === 'failed' ? 'Failed' : 'Pending'}
        </h1>
        <p className="font-body-lg text-body-lg text-on-surface-variant mb-2">
          {status === 'failed'
            ? 'Your enrollment could not be completed. Please try again.'
